...
- `~/.juliaup/bin/julia --project=. -e 'using Pkg; Pkg.test(test_args=["--jobs=4"])'` runs the suite in parallel with 4 workers.
- `~/.juliaup/bin/julia --project=. -e 'using Pkg; Pkg.test(test_args=["--list"])'` lists autodiscovered test files.
- `~/.juliaup/bin/julia --project=. -e 'using Pkg; Pkg.test(test_args=["training/test_train_loop"])'` runs a filtered test file.
...
## Testing Guidelines
Add tests in the mirrored area under `test/`; `ParallelTestRunner.jl` autodiscovers test files, so new groups do not need manual registration in `test/runtests.jl`. Prefer focused unit tests for module logic and add or update integration coverage when changing experiment flows or recovery behavior. There is no formal coverage gate, but changes should pass targeted tests and the full `Pkg.test()` suite before review.
